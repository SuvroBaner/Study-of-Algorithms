...
def poprandom(nodes):
    x_i = randint(0, len(nodes) - 1)  # note, here x_i is the index
...
def pickrandom(nodes):
    x_i = randint(0, len(nodes) - 1)  # here x_i is an index
    return nodes[x_i]
...
def create_tour(nodes):
    ...
    # then pick a random node from the unconnected list
    ...
    # because the sum of the degrees of a graph is even
    ...
    while len(odd_nodes) > 0:
        ...
        if cn is not None:
            ...
            # if we get here, the nodes is already connected to all the odd nodes so we need to 
            # find an even one to connect to
            cn = check_nodes(x, even_nodes, tour)
            # cn can not be None and needs to be added to the odd_nodes list
            odd_nodes.append(cn)
